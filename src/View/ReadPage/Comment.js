...
const CommentComponent = ({ bookid, page }) => {
    ...
    return (
        <div>
            <div style={{ maxHeight: `${VISIBLE_COMMENTS * 125}px`, overflowY: 'scroll' }}>
                    ...
                    <React.Fragment key={comment.page}>
                        <div
                            ...
                            style={{
                                ...
                                width: '403px',
                                borderColor: '#DBDBDB',
                                borderWidth: index === 0 ? '1px 2px 1px 2px' : (index === comments.length - 1 ? '1px 2px 1px 2px' : '1px 2px 1px 2px'),
                                borderStyle: 'solid',
                            ...
                            <div style={{ color: '#3477CF', fontSize: '25px', marginBottom: '15px', textAlign: 'left', display: 'flex', justifyContent: 'space-between' }}>
                                <b>Page {comment.page}</b>
                                <span style={{ color: '#B1B1B1', fontSize: '18px', marginRight: '10px' }}>{comment.time}</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '15px' }}>
                                <span style={{ color: '#244F8D', fontSize: '22px', textAlign: 'left' }}>{comment.nickname}</span>
                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                    <img
                                        src={comment.isLiked ? filledHeart : emptyheart}
                                        alt="Heart Icon"
                                        style={{ width: '23px', height: '23px', cursor: 'pointer', marginRight: '10px' }}
                                        onClick={() => {
                                            const updatedComments = [...comments];
                                            if (comment.isLiked) {
                                    ...
                                    />
                                    <span style={{ fontSize: '18px', marginRight: '10px' }}>{comment.likes}</span>
                                </div>
                            </div>

                            <div style={{ color: '#545454', fontSize: '20px', marginBottom: '15px', textAlign: 'left' }}>
                            ...
                            {comment.replies && comment.replies.map((reply, rIndex) => (
                                <div key={rIndex} style={{
                                    ...
                                    <div style={{ color: '#3477CF', fontSize: '25px', marginBottom: '15px', textAlign: 'left', display: 'flex', justifyContent: 'space-between' }}>
                                        <img src={recomment_arrow} alt="Reply Arrow" style={{ width: '25px', marginRight: '10px' }} />
                                        <span style={{ color: '#B1B1B1', fontSize: '18px', marginRight: '10px' }}>{reply.time}</span>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '15px' }}>
                                        <span style={{ color: '#244F8D', fontSize: '22px', textAlign: 'left' }}>{reply.nickname}</span>
                            ...
                            {replyTo === index && (
                                <div style={{ marginLeft: '10px', marginTop: '20px', display: 'flex', flexDirection: 'column' }}>
                                    <textarea
                                        ...
                                        maxLength={80}
                                    ></textarea>
                                    <div style={{ marginTop: '10px', marginRight: '7px', display: 'flex', justifyContent: 'flex-end' }}>
                                        <button onClick={handleReplySubmit} style={{
                                            background: '#3477CF',
                                            color: 'white',
                                            width: '60px',
                                            height: '35px',
            ...
            {totalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '20px' }}>
                    <img src={leftArrow} alt="left arrow" style={{ cursor: 'pointer' }} onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} />
                    ...
                    {Array.from({ length: totalPages }).map((_, index) => (
                        <button
                            ...
                            style={{
                                margin: '0 5px',
                                cursor: 'pointer',
                                backgroundColor: currentPage === index + 1 ? '#3477CF' : 'transparent',
                    ...
                    <img src={rightArrow} alt="right arrow" style={{ cursor: 'pointer' }} onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} />
                </div>
            )}
            ...
            <div style={{ marginTop: '70px', marginLeft: '30px' }}>
                <textarea
                    ...
                    maxLength={80}
                ></textarea>
                <div style={{ marginTop: '10px', marginRight: '8px', display: 'flex', justifyContent: 'flex-end' }}>
